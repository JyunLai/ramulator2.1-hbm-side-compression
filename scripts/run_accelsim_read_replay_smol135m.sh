...
OUT=results/accelsim_read_replay_smol135m_ch16.csv
mkdir -p results logs_accelsim_replay

echo "model,seq_len,channels,ramulator_cycles" > "$OUT"

for L in 128 512 1024 2048; do
  TRACE=$PWD/traces_accelsim_replay/smol135m_seq${L}_reads_ch16.trace
  LOG=$PWD/logs_accelsim_replay/smol135m_seq${L}_reads_ch16.log

  echo "===== Ramulator replay SmolLM2-135M seq=$L ch=16 =====" >&2
  ...
  echo "SmolLM2-135M,$L,16,$cycles" | tee -a "$OUT"
done
